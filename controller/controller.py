...
class Controller:
    ...
    def calc_restr(self):
        ...
        if self.reset_relations:
            self.lprp = LpRecipeProblem('Recipe Problem', self.mod.current_recipe, self.mod)
            self.reset_relations = False
    ...
    def update_ingredient_dict(self):
        ...
        for i, ing in self.mod.ingredient_dict.items():
            ...
            for j, attr in self.mod.attr_dict.items():
                try:
                    val = eval(self.ing_editor.display_ingredients[i].attr_entry[j].get())
                except:
                    val = 0
                if isinstance(val, Number) and val != 0:
                    ing.attributes[j] = val
                else:
                    self.ing_editor.display_ingredients[i].attr_entry[j].delete(0, tk.END)
                    try:
                        del ing.attributes[j]
                    except:
                        pass
